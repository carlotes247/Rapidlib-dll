#include "RapidLibPlugin_ARM.h"

#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "RapidLibPlugin_ARM", __VA_ARGS__))
#define LOGW(...) ((void)__android_log_print(ANDROID_LOG_WARN, "RapidLibPlugin_ARM", __VA_ARGS__))

#define EXPORT_API // Android may not need annotating exported functions, so define is empty
...
extern "C" {
	...
	const char * RapidLibPlugin_ARM::getPlatformABI()
	{
	#if defined(__arm__)
	...
	void RapidLibPlugin_ARM()
	{
	}

	RapidLibPlugin_ARM::RapidLibPlugin_ARM()
	{
	}
...
//
//		strcpy(combinedText, parameterText);
//		strcat(combinedText, additionalText);
//
